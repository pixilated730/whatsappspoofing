...
func handler(rawEvt interface{}) {
	...
	case *events.Message:
		...
		log.Infof("DEBUG_INFO: %+v", evt.Info)
		...
		//if strings.HasPrefix(msg, "/rfds") {
		//	recipient := evt.Info.Chat
		//	msg := &waProto.Message{
		//		ExtendedTextMessage: &waProto.ExtendedTextMessage{
		//			Text: proto.String("Não acredite nele, eu sou o verdadeiro!"),
		...
		//	resp, err := cli.SendMessage(context.Background(), recipient, msg)
		//	if err != nil {
		//		log.Errorf("Error sending message: %v / %+v", err, msg)
		//	} else {
		//		log.Infof("Message sent (server timestamp: %s): %+v", resp.Timestamp, msg)
		//	}
		//}
		...
		if strings.HasPrefix(msg, "/lichti.now") {
			recipient := evt.Info.Chat
		...
		if sticker != nil {
			...
			log.Infof("Saved sticker in message to %s", path)
		}

		contact := evt.Message.GetContactMessage()
		if contact != nil {
			path := fmt.Sprintf("%s%s", evt.Info.ID, ".vcf")
			err := os.WriteFile(path, []byte(*contact.Vcard), 0600)
			if err != nil {
				log.Errorf("Failed to save contact: %v", err)
				return
			}
			log.Infof("Saved contact in message to %s", path)
		}	
